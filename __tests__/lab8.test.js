describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    console.log(`Checking product item 1/${products.length}`);
    // Grab the .data property of <product-items> to grab all of the json data stored inside
    for(let i = 0; i < products.length; i++) {
      data = await products[i].getProperty('data');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < products.length; i++) {
      ...
      const root = await data.getProperty('shadowRoot');
      const button = await root.$("button");
      await button.click();
    }
    ...
    expect(await text.jsonValue()).toEqual('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 1; i < products.length; i++) {
      ...
      expect(await innerText.jsonValue()).toEqual('Add to Cart');
    }
    // Also check to make sure that #cart-count is still 0
    const count = await page.$("#cart-count");
    const innerText = await count.getProperty('innerText');
    expect(await innerText.jsonValue()).toEqual('0');
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const localCart = await page.evaluate(() => {return localStorage.getItem('cart');});
